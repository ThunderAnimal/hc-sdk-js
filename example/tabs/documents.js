...
function createUpdateForm(userId, hcDocument, display) {
  ...
  updateButton.addEventListener("click", () => {
    ...
    HC.updateDocument(userId, hcDocument).then(() => {
      display(hcDocument);
    });
  });
  ...
  deleteButton.addEventListener("click", () => {
    HC.deleteDocument(userId, hcDocument).then(() => {
      resultElement.innerHTML = `Document '${
        hcDocument.id
...
function displayDocument(hcDocument) {
  ...
  resultElement.appendChild(
    createUpdateForm(currentUserId, hcDocument, displayDocument)
  );
  ...
  hcDocument.attachments.forEach(attachment => {
    ...
    deleteFile.addEventListener("click", () => {
      hcDocument.attachments = hcDocument.attachments.filter(
...
function getDocument(documentId) {
  HC.downloadDocument(HC.getCurrentUser().id, documentId).then(displayDocument);
}

function uploadDocument(files, title, author) {
  let hcDocument = new HC.models.HCDocument({ files, title, author });

  HC.uploadDocument(HC.getCurrentUser().id, hcDocument).then(displayDocument);
}
