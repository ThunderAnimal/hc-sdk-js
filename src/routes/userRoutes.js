...
const userRoutes = {
	...
	getGrantedPermissions(userId, granteeId) {
		return hcRequest('GET', `${apiUrl}/users/${userId}/permissions`, { authorize: true, query: { grantee_id: granteeId } });
	},
	...
	verifyShareAndGrantPermission(ownerId, granteeId, OperationId) {
		...
		return hcRequest('POST', `${apiUrl}/users/${ownerId}/permissions`, { body, authorize: true });
	},

	addTagEncryptionKey(userId, secret) {
		const body = { tek: secret };

		return hcRequest('POST', `${apiUrl}/users/${userId}/tek`, { body, authorize: true });
	},

	updateUser(userId, userData) {
		return hcRequest('PUT', `${apiUrl}/users/${userId}`, { body: userData, authorize: true });
	},
};
