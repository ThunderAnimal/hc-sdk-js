...
class DocumentService {
  ...
  downloadDocument(ownerId, documentId) {
    ...
    return this.fhirService
      ...
      .then(encryptedBlobs =>
        Promise.all(
          encryptedBlobs.map(encryptedBlob =>
            this.zeroKitAdapter.decryptBlob(encryptedBlob)
          )
        )
      )
      .then(blobs => {
  ...
  uploadDocument(ownerId, hcDocument) {
    ...
    return this.fhirService
      .createFhirRecord(ownerId, hcDocumentUtils.toFhirObject(hcDocument))
      .then(record => {
  ...
  updateDocument(ownerId, hcDocument) {
    ...
    newAttachments = newAttachments.map(attachment =>
      this.zeroKitAdapter
        .encryptBlob(ownerId, attachment.file)
    ...
    return Promise.all([
      ...
      newAttachments.length
        ? documentRoutes.getFileUploadUrls(
            ownerId,
            hcDocument.id,
            newAttachments.length
          )
  ...
  getDocuments(ownerId, params = {}) {
    ...
    return this.fhirService.searchRecords(ownerId, params).then(result => {
