...
import {
  ...
  tagIds as loginFormIds
} from "../templates/loginForm";
import formStyles from "../templates/formStyles";
import {
  createRegistrationFormTemplate,
  tagIds as registrationFormIds
} from "../templates/registrationForm";
...
class ZeroKitAdapter {
  ...
  loginNode(hcUserAlias, password) {
    return userService.resolveUser(hcUserAlias).then(user =>
      this.zeroKit
        .then(zeroKit => zeroKit.login(user.zeroKitId, password))
  ...
  login(zKitLoginObject, hcUserAlias) {
    let userId;
    return userService
      .resolveUser(hcUserAlias)
      .then(user => {
        const { zeroKitId } = user;
        userId = user.id;
        sessionHandler.setItem("HC_User", `${userId},${hcUserAlias}`);

        return zKitLoginObject.then(loginObject =>
          loginObject.login(zeroKitId)
        );
      })
  ...
  setupUser() {
    ...
    return userService
      ...
      .then(user => {
        ...
        if (!tresorId) {
          return this.createTresor().then(createdTresorId => {
      ...
      .then(tresorId => {
        if (!tek) {
          this.createTek(tresorId).then(createdTek => {
  ...
  getLoginForm(parentElement) {
    return new Promise((resolve, reject) => {
      ...
      const zKitLoginObject = this.zeroKit.then(zeroKit => {
      ...
      const submit = function(zKitLogin, event) {
        ...
        this.login(
          zKitLogin,
          document.getElementById(loginFormIds.hcUserLogin).value
        )
          .then(response => {
  ...
  createTek(tresorId) {
    ...
    return this.zeroKit.then(zeroKit =>
      zeroKit
        .encrypt(tresorId, tek)
        .then(res =>
          userRoutes
            .addTagEncryptionKey(userService.getCurrentUser().id, res)
            .then(() => tek)
        )
    );
  }
  ...
  getRegistrationForm(parentElement) {
    return new Promise((resolve, reject) => {
      ...
      const zkitRegisterNode = document.getElementById(
        registrationFormIds.zkitRegistration
      );
      while (zkitRegisterNode.firstChild) {
        zkitRegisterNode.removeChild(zkitRegisterNode.firstChild);
      }
      this.zKitRegistrationObject = this.zeroKit
        .then(zeroKit => {
          stylesUtils.appendStyles(formStyles);
          resolve();
  ...
  register(autoLogin = false) {
    const hcUserAlias = document.getElementById(
      registrationFormIds.hcUserRegister
    ).value;
    ...
    return userRoutes
      ...
      .then(registrationObject =>
        registrationObject.register(zKitId, sessionId)
      )
      .then(res =>
        userRoutes.validateRegistration(res.RegValidationVerifier, zKitId)
      )
      .then(
        () =>
          autoLogin
  ...
  encrypt(ownerId, plainText) {
    return userService
      .getInternalUser(ownerId)
      .then(owner =>
        this.zeroKit.then(zeroKit => zeroKit.encrypt(owner.tresorId, plainText))
      );
  }
  ...
  createTresor() {
    return this.zeroKit.then(zeroKit =>
      zeroKit
        ...
        .then(tresorId =>
          userRoutes
            .addTresor(userService.getCurrentUser().id, tresorId)
            .then(() => tresorId)
        )
    );
  }
  ...
  grantPermission(granteeAlias) {
    ...
    return Promise.all([
      ...
      .then(operationId => {
        userRoutes.verifyShareAndGrantPermission(
          owner.id,
          grantee.id,
          operationId
        );
