...
const getCodeFromString = (queryString, key) =>
  queryString
...
class Auth {
  ...
  idpLogin() {
    return new Promise((resolve, reject) => {
      ...
      iframe.onload = Auth.handleIframe.bind(
        this,
        iframe,
        (error, queryString) => {
          ...
          sessionHandler.setItem(
            "HC_Id",
            getCodeFromString(queryString, "auth_token")
          );
          this.authorize()
  ...
  clientCredentialsLogin(userId) {
    ...
    return authRoutes.getAccessTokenFromCredentials(body).then(res => {
  ...
  static logout() {
    return authRoutes
      .revokeRefreshToken(sessionHandler.getItem("HC_Refresh"))
