...
class FHIRService {
  ...
  updateFhirRecord(ownerId, recordId, fhirObject) {
    const updateRequest = (userId, params) =>
      documentRoutes.updateRecord(userId, recordId, params);

    return this.downloadFhirRecord(ownerId, recordId).then(record => {
  ...
  uploadFhirRecord(ownerId, fhirObject, uploadRequest) {
    return fhirValidator.validate(fhirObject).then(() => {
      const tags = [
        ...taggingUtils.generateTagsFromFhirObject(fhirObject),
        taggingUtils.buildTag(
          "client",
    ...
    return userService
      ...
      .then(userObject => {
        ...
        if (params.client_id) {
          ...
          params.tags = params.tags ? [...params.tags, clientTag] : [clientTag];
          delete params.client_id;
        }

        if (params.tags) {
          params.tags = params.tags
            .map(tag => encryptionUtils.encrypt(tag, user.tek))
            .join(",");
        }
      ...
      .then(
        results => (results ? { totalCount, records: results } : { totalCount })
      );
  }
