...
const userService = {
  ...
  getUser(userId) {
    return this.getInternalUser(userId).then(userDetails => ({
      id: userDetails.id,
    ...
    return new Promise((resolve, reject) => {
      ...
      this.getInternalUser(currentUser.id, true)
        ...
        .then(encryptedUserDetails =>
          userRoutes.updateUser(currentUser.id, {
            encrypted_data: encryptedUserDetails
          })
        )
        .then(resolve)
        .catch(reject);
  ...
  getGrantedPermissions(granteeId) {
    ...
    return userRoutes
      .getGrantedPermissions(owner.id, granteeId)
      .then(permissions =>
