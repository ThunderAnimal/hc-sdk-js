...
describe("auth Routes", () => {
  ...
  beforeEach(() => {
    ...
    document.cookie =
      "SQLiteManager_currentLangue=2; HC_User=c7212d03-34e9-40ab-9029-ed66" +
      "2ecb7f0b,1";
    ...
    credentialsStub
      .withArgs("POST", `${config.api}/auth/token`, {
        body: expectedParamsForClientCredsGrantType,
        headers: expectedHeadersForClientCredsGrantType
      })
      .resolves("pass");
  ...
  it("getAccessTokenFromCredentials passes", done => {
    ...
    authRoutes
      .getAccessTokenFromCredentials(params)
      .then(res => {
        expect(res).to.equal("pass");
  ...
  it("getAccessTokenFromCredentials fails when wrong client_secret is passed", done => {
    ...
    authRoutes
      .getAccessTokenFromCredentials(actualParams)
      .then(() =>
        done(
          Error(
            "getAccessTokenFromCredentials rejection didn't work properly in authRoutes.getAccessTokenFromCredentials"
          )
        )
      )
      .catch(err => {
  ...
  it("revokeAccesToken - Happy Path", done => {
    authRoutes
      .revokeRefreshToken(testVariables.refreshToken)
      .then(() => {
        expect(requestStub).to.be.calledOnce;
        expect(requestStub.firstCall.args[0]).to.equal("POST");
        expect(requestStub.firstCall.args[2].body).to.deep.equal({
          token: testVariables.refreshToken
        });
        done();
      })
      .catch(done);
