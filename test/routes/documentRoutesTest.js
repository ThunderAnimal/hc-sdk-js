...
describe("documentRoutes", () => {
  ...
  it("getFileDownloadUrl passes", done => {
    ...
    documentRoutes
      .getFileDownloadUrl("fakeUserAlias", "fakeDocumentId")
      .then(res => {
  ...
  it("getFileUploadUrls passes", done => {
    ...
    documentRoutes
      .getFileUploadUrls("fakeUserAlias", "fakeRecordId", "42")
      .then(res => {
        expect(res).to.equal("pass");
        expect(requestStub).to.be.calledOnce;
        expect(requestStub.firstCall.args[2].authorize).to.equal(true);
  ...
  it("updateRecordStatus passes", done => {
    ...
    documentRoutes
      .updateRecordStatus("fakeUserId", "fakeRecordId", "Active")
      .then(res => {
        expect(res).to.equal("pass");
        expect(requestStub).to.be.calledOnce;
        expect(requestStub.firstCall.args[2].authorize).to.equal(true);
  ...
  it("searchRecord passes", done => {
    ...
    requestStub
      .withArgs("GET", `${config.api}/users/${testVariables.userId}/records`, {
        query: searchParmas,
        authorize: true,
    ...
    documentRoutes
      .searchRecords(testVariables.userId, searchParmas)
      .then(res => {
        expect(requestStub).to.be.calledOnce;
  ...
  it("getRecordCount passes", done => {
    const searchParmas = { tags: [testVariables.tag] };
    requestStub
      .withArgs("HEAD", `${config.api}/users/${testVariables.userId}/records`, {
        query: searchParmas,
        authorize: true,
        includeResponseHeaders: true
      })
