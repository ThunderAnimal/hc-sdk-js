...
describe("models/document", () => {
  ...
  it(
    "converting a document to fhir and back doesn't" +
      " change the document(except the attachments)",
    () => {
      const hcDocument = new HCDocument(documentParameters);
      const fhirDocument = hcDocumentUtils.toFhirObject(hcDocument);
      const fhirGeneratedDocument = hcDocumentUtils.fromFhirObject(
        fhirDocument
      );
      expect(fhirGeneratedDocument.attachments.length).to.equal(
        hcDocument.attachments.length
      );
