...
describe("sessionHandler web", () => {
  ...
  describe("localStorage available", () => {
    beforeEach(() => {
      ...
      sessionHandler = proxyquire("../../../src/lib/sessionHandler/web", {
        "../environmentUtils": {
          default: {
            getLocalStorage: sinon.stub().returns(window.localStorage)
          }
        }
      }).default;
    });
    ...
    it("should be possible to set a given value for a given key", () => {
      ...
      expect(window.localStorage.getItem("HC_User")).to.equal(
        `${testVariables.userId},${testVariables.userAlias}`
      );
    });
  ...
  describe("localStorage not available", () => {
    beforeEach(() => {
      sessionHandler = proxyquire("../../../src/lib/sessionHandler/web", {
        "../environmentUtils": {
          default: { getLocalStorage: sinon.stub().returns(undefined) }
        }
      }).default;
    });

    it("should be possible to get the value for a given key", () => {
      document.cookie = `HC_User=${testVariables.userId},${
        testVariables.userAlias
      }`;
