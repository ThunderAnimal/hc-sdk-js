...
describe("fhir validator", () => {
  ...
  beforeEach(() => {
    fhirRouteStub = sinon
      .stub(fhirRoutes, "getFhirSchema")
      .returnsPromise()
      .resolves(JSON.stringify(schema));
  });

  it("getConformance succeeds", done => {
    fhirValidator.getConformance().then(res => {
  ...
  it("isValidResourceType succeeds", done => {
    fhirValidator.isValidResourceType("DocumentReference").then(res => {
  ...
  it("isValidResourceType fails for invalid resource", done => {
    fhirValidator.isValidResourceType("Document").catch(err => {
  ...
  it("validate succeeds", done => {
    ...
    fhirValidator.validate(dummyObject).then(res => {
      expect(res).to.not.be.null;
      done();
    });
  });
  ...
  it("validate fails if invalid resource type", done => {
    ...
    fhirValidator.validate(dummyObject).catch(err => {
      expect(err).to.not.be.null;
