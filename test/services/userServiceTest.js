...
import {
  MISSING_PARAMETERS,
  INVALID_PARAMETERS
} from "../../src/lib/errors/ValidationError";
...
describe("services/userService", () => {
  ...
  beforeEach(() => {
    ...
    decryptStub
      .withArgs(testVariables.encryptedUserData)
      .returns(Promise.resolve(JSON.stringify(testVariables.userData)));
    decryptStub
      .withArgs(testVariables.encryptedTek)
      .returns(Promise.resolve(testVariables.tek));
    encryptStub = sinon
      .stub()
      .returnsPromise()
      .resolves(testVariables.encryptedUserData);
    getGrantedPermissionsStub = sinon
      .stub(userRoutes, "getGrantedPermissions")
      .returnsPromise()
      .resolves([{ owner_id: "a", grantee_id: "b" }]);
    getUserDetailsStub = sinon
      .stub(userRoutes, "getUserDetails")
      .returnsPromise()
      .resolves(userResources.userDetails);
    resolveUserIdStub = sinon
      .stub(userRoutes, "resolveUserId")
      .returnsPromise()
      .resolves({
        uid: testVariables.userId,
        zerokit_id: testVariables.zeroKitId
      });
    sessionHandlerGetStub = sinon
      .stub(sessionHandler, "getItem")
      .returns(`${testVariables.userId},${testVariables.userAlias}`);
    updateUserStub = sinon
      .stub(userRoutes, "updateUser")
      .returnsPromise()
      .resolves();
  ...
  it("resolveUser succeeds", done => {
    userService
      .resolveUser(testVariables.userAlias)
      .then(res => {
  ...
  it("getUserIdForAlias - Happy Path", done => {
    userService
      .getUserIdForAlias(testVariables.userAlias)
      .then(res => {
  ...
  it("updateUser - fails when parameters is not an object", done => {
    userService.updateUser("string").catch(res => {
      expect(res.message).to.equal(
        `${INVALID_PARAMETERS}: parameter is not an object`
      );
      done();
    });
  });
