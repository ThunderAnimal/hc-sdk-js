...
describe("zerokitAdapter", () => {
  ...
  beforeEach(() => {
    addTagEncryptionKeyStub = sinon
      .stub(userRoutes, "addTagEncryptionKey")
      .returnsPromise()
      .resolves();
    addTresorStub = sinon
      .stub(userRoutes, "addTresor")
      .returnsPromise()
      .resolves();
    authServiceLogoutStub = sinon
      .stub()
      .returnsPromise()
      .resolves();
    createTresorStub = sinon
      .stub()
      .returnsPromise()
      .resolves(testVariables.tresorId);
    decryptStub = sinon
      .stub()
      .returnsPromise()
      .resolves(testVariables.string);
    encryptStub = sinon
      .stub()
      .returnsPromise()
      .resolves(testVariables.encryptedString);
    getCurrentUserStub = sinon.stub().returns(userResources.currentUser);
    getElementByIdStub = sinon
      .stub(window.document, "getElementById")
      .returns({ value: testVariables.userAlias });
    getInternalUserStub = sinon
      .stub()
      .returnsPromise()
      .resolves(userResources.internalUser);
    idpLoginStub = sinon
      .stub()
      .returnsPromise()
      .resolves();
    initRegistrationStub = sinon
      .stub(userRoutes, "initRegistration")
      .returnsPromise()
      .resolves({
        session_id: testVariables.sessionId,
        zerokit_id: testVariables.zeroKitId
      });
    loginStub = sinon
      .stub()
      .returnsPromise()
      .resolves(testVariables.zeroKitId);
    loginNodeStub = sinon
      .stub()
      .returnsPromise()
    ...
    registerStub = sinon
      .stub()
      .returnsPromise()
      .resolves({ RegValidationVerifier: testVariables.regValidationVerifier });
    resolveUserStub = sinon
      .stub()
      .returnsPromise()
      .resolves(userResources.resolvedUser);
    sessionHandlerSetStub = sinon.stub(sessionHandler, "set").returns();
    sessionHandlerLogoutStub = sinon.stub(sessionHandler, "logout").returns();
    shareTresorStub = sinon
      .stub()
      .returnsPromise()
      .resolves(testVariables.operationId);
    validateRegistrationStub = sinon
      .stub(userRoutes, "validateRegistration")
      .returnsPromise()
      .resolves();
    verifyShareAndGrantPermissionStub = sinon
      .stub(userRoutes, "verifyShareAndGrantPermission")
      .returnsPromise()
      .resolves();
    ...
    getLoginIframeStub = sinon
      .stub()
      .returnsPromise()
      .resolves(zKitLoginObject);
    zeroKit = {
  ...
  it("login - createsTresor and tek on first login", done => {
    ...
    zerokitAdapter
      .login(zKitLoginObjectPromise, testVariables.userAlias)
      .then(res => {
  ...
  it("login - callback rejects when resolveUser call fails ", done => {
    ...
    zerokitAdapter
      ...
      .then(() =>
          ...
          Error(
            "resolveUserStub rejection didn't work properly in zerokitAdapter.login"
          )
        )
      )
      .catch(() => {
  ...
  it("loginNode - createsTresor and tek on first login", done => {
    ...
    zerokitAdapterNode
      .loginNode(testVariables.userAlias, testVariables.password)
      .then(() => {
  ...
  it("loginNode - rejects when wrong zerokit password is entered", done => {
    ...
    zerokitAdapterNode
      .loginNode(testVariables.userAlias, testVariables.wrongPassword)
      .catch(() => {
        expect(zeroKitNode.login).to.be.calledOnce;
        expect(zeroKitNode.createTresor).to.not.be.called;
        done();
      ...
      .then(() =>
          ...
          Error(
            "loginNode rejection didn't work properly in zerokitAdapter.loginNode"
          )
        )
      );
  });
  ...
  it("register - rejects when userAlias is not an email", done => {
    getElementByIdStub.returns({
      value: testVariables.userAlias.split("@")[0]
    });
    zerokitAdapter
      .register()
      .then(() =>
          ...
          Error(
            "email validation didn't work properly in zerokitAdapter.register"
          )
        )
      )
      .catch(err => {
  ...
  it("register - rejects when initRegistration fails", done => {
    ...
    zerokitAdapter
      ...
      .then(() =>
          ...
          Error(
            "initRegistrationStub rejection didn't work properly in zerokitAdapter.register"
          )
        )
      )
      .catch(() => {
  ...
  it("encrypt - Happy Path", done => {
    zerokitAdapter
      .encrypt(testVariables.userId, testVariables.string)
      .then(res => {
        expect(encryptStub).to.be.calledOnce;
        expect(encryptStub).to.be.calledWith(
          testVariables.tresorId,
  ...
  it("decrypt - Happy Path", done => {
    zerokitAdapter
      .decrypt(testVariables.encryptedString)
      .then(res => {
  ...
  it("getLoginForm - Happy Path", done => {
    ...
    const createLoginFormStub = sinon
      .stub(loginForm, "createLoginFormTemplate")
      .returns(createdLoginForm);
  ...
  it("logout - rejects when zerokit logout fails", done => {
    ...
    zerokitAdapter
      ...
      .then(() =>
          ...
          Error(
            "logoutStub rejection didn't work properly in zerokitAdapter.logout"
          )
        )
      )
      .catch(() => {
  ...
  it("grantPermission - Happy Path", done => {
    zerokitAdapter
      .grantPermission(testVariables.userAlias)
      .then(() => {
        expect(resolveUserStub).to.be.calledOnce;
  ...
  it("grantPermission - swallows error when zerokit returns AlreadyMember error", done => {
    ...
    zerokitAdapter
      .grantPermission("fakeGranteeEmail")
      .then(() => {
        expect(resolveUserStub).to.be.calledOnce;
        expect(shareTresorStub).to.be.calledOnce;
        expect(getInternalUserStub).to.be.calledOnce;
  ...
  it("grantPermission - rejects when zeroKits shareTresor fails", done => {
    ...
    zerokitAdapter
      ...
      .then(() =>
          ...
          Error(
            "shareTresorStub rejection didn't work properly in zerokitAdapter.grantPermission"
          )
        )
      )
      .catch(() => {
