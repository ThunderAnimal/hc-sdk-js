...
describe("services/fhirService", () => {
  ...
  beforeEach(() => {
    createRecordStub = sinon
      ...
      .returnsPromise()
      .resolves(Object.assign({}, recordResources.documentReference));
    decryptStub = sinon
      .stub(ZeroKitAdapter, "decrypt")
      .returnsPromise()
      .resolves(
        JSON.stringify(Object.assign({}, fhirResources.documentReference))
      );
    deleteRecordStub = sinon
      .stub(documentRoutes, "deleteRecord")
      .returnsPromise()
      .resolves();
    downloadRecordStub = sinon
      .stub(documentRoutes, "downloadRecord")
      .returnsPromise()
      .resolves(Object.assign({}, recordResources.documentReferenceEncrypted));
    encryptStub = sinon
      .stub(ZeroKitAdapter, "encrypt")
      .returnsPromise()
      .resolves("encrypted_body");
    generateTagsFromFhirObjectStub = sinon
      .stub(taggingUtils, "generateTagsFromFhirObject")
      .returns([testVariables.tag]);
    getInternalUserStub = sinon
      .stub(userService, "getInternalUser")
      .returnsPromise()
      .resolves(userResources.internalUser);
    getRecordsCountStub = sinon
      .stub(documentRoutes, "getRecordsCount")
      .returnsPromise()
      .resolves({
        totalCount: recordResources.count
      });
    searchRecordsStub = sinon
      .stub(documentRoutes, "searchRecords")
      .returnsPromise()
      .resolves({
        totalCount: recordResources.count,
        records: [Object.assign({}, recordResources.documentReferenceEncrypted)]
      });
    tagDecryptStub = sinon
      .stub(encryptionUtils, "decrypt")
      .returns(testVariables.tag);
    tagEncryptStub = sinon
      .stub(encryptionUtils, "encrypt")
  ...
  it("createFhirRecord - Happy Path", done => {
    fhirService
      .createFhirRecord(testVariables.userId, fhirResources.documentReference)
      .then(() => {
        expect(createRecordStub).to.be.calledOnce;
        expect(tagEncryptStub.getCall(1).args[0]).to.equal(
          `client=${stringUtils.addPercentEncoding(testVariables.clientId)}`
        );
  ...
  it("createFhirRecord - fails when fhirValidation fails", done => {
    ...
    fhirService
      .uploadFhirRecord(testVariables.userId, fhirResources.documentReference)
  ...
  it("downloadFhirRecord - Happy Path", done => {
    fhirService
      .downloadFhirRecord(testVariables.userId, testVariables.recordId)
      .then(res => {
  ...
  it("deleteRecord - Happy Path", done => {
    FhirService.deleteRecord(testVariables.userId, testVariables.recordId)
      .then(() => {
        ...
        expect(deleteRecordStub).to.be.calledWith(
          testVariables.userId,
          testVariables.recordId
        );
        done();
