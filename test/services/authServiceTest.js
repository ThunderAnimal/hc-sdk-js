...
describe("AuthService", () => {
  ...
  beforeEach(() => {
    ...
    getAccessTokenFromCodeStub = sinon
      .stub(authRoutes, "getAccessTokenFromCode")
      .returnsPromise()
    ...
    getAccessTokenFromCredentialsStub = sinon
      .stub(authRoutes, "getAccessTokenFromCredentials")
      .returnsPromise()
      .withArgs(expectedParamsForClientCredentials)
      .resolves(accessTokenObject);

    logoutStub = sinon.stub(sessionHandler, "logout").returns();
    revokeRefreshTokenStub = sinon
      .stub(authRoutes, "revokeRefreshToken")
      .returnsPromise()
      .resolves();
  });

  it("authorize() succeeds", done => {
    const handleIframeStub = sinon
      .stub(AuthService, "handleIframe")
  ...
  it("idpLogin succeeds", done => {
    ...
    authService
      .idpLogin()
      .then(res => {
        expect(handleIframeStub.getCall(0).args[0].src.toLowerCase()).to.equal(
          `${config.api}/login?client_id=${
            testVariables.zeroKitId
          }&reto=${encodeURIComponent(location.href)}`.toLowerCase()
        );
        expect(res.access_token).to.equal("fake_access_token");
  ...
  it("clientCredentialsLogin succeeds", done => {
    authService
      .clientCredentialsLogin(testVariables.userId)
      .then(() => {
