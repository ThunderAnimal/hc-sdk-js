...
describe("DocumentService", () => {
  ...
  const hcDocumentWithFileData = JSON.parse(
    JSON.stringify(hcDocumentWithoutFileData)
  );
  ...
  const documentReferenceRecordFactory = (
    files = [],
    basicRecord = documentReferenceRecord
  ) => {
  ...
  beforeEach(() => {
    // stubs
    getFileUploadUrlsStub = sinon
      .stub(documentRoutes, "getFileUploadUrls")
      .returnsPromise();
    getFileDownloadUrlStub = sinon
      .stub(documentRoutes, "getFileDownloadUrl")
      .returnsPromise();
    ...
    updateRecordStatusStub = sinon
      .stub(documentRoutes, "updateRecordStatus")
      .returnsPromise();
    ...
    decryptBlobStub = sinon
      .stub()
      .returnsPromise()
      .resolves(file);
    encryptBlobStub = sinon
      .stub()
      .returnsPromise()
      .withArgs(file)
      .resolves(encryptedFile);
    ...
    documentService.fhirService.createFhirRecord = sinon
      .stub()
      .returnsPromise()
      .resolves(documentReferenceRecord);
    documentService.fhirService.downloadFhirRecord = sinon
      .stub()
      .returnsPromise()
      .resolves(documentReferenceRecordFactory([encryptedFile]));
    documentService.fhirService.updateFhirRecord = sinon
      .stub()
      .returnsPromise()
      .resolves({ record_id: recordId });
    documentService.fhirService.searchRecords = sinon
      .stub()
      .returnsPromise()
      ...
      .resolves({
        totalCount: 1,
        records: [recordResources.documentReference]
      });
  });
  ...
  it("uploadDocument - Happy Path", done => {
    ...
    documentService.uploadDocument(userId, hcDocumentWithFileData).then(() => {
      ...
      expect(documentService.fhirService.updateFhirRecord).to.be.calledWith(
        userId,
        recordId,
        fhirObject
      );
  ...
  it("uploadDocument - rejects when getFileUploadUrls fails ", done => {
    ...
    documentService
      .uploadDocument(userId, hcDocumentWithFileData)
      .catch(err => {
  ...
  it("updateDocument - Happy Path with no changes", done => {
    ...
    documentService
      .updateDocument(userId, hcDocumentWithoutFileData)
      .then(() => {
  ...
  it("updateDocument - fails when getFileUploadUrls fails ", done => {
    getFileUploadUrlsStub.rejects("error");
    documentService
      .updateDocument(userId, hcDocumentWithFileData)
      .catch(err => {
        expect(err).to.equal("error");
        expect(getFileUploadUrlsStub).to.be.calledOnce;
        expect(uploadFileStub).to.be.not.called;
  ...
  describe("getDocuments", () => {
    it("should pass", done => {
      fromFhirObjectStub.returns(hcDocumentWithoutFileData);
      documentService
        .getDocuments(testVariables.userId)
        .then(result => {
    ...
    it("should fail if searchrecords return error", done => {
      ...
      documentService.fhirService.searchRecords = sinon
        ...
        .returnsPromise()
        .rejects("error");
      documentService.getDocuments(testVariables.secondUserId).catch(err => {
        expect(err).to.equal("error");
  ...
  describe("getDocumentsCount", () => {
    it("should pass", done => {
      ...
      documentService
        .getDocumentsCount(testVariables.userId)
        .then(result => {
    ...
    it("should fail if searchrecords return error", done => {
      ...
      documentService.fhirService.searchRecords = sinon
        ...
        .rejects("error");
      documentService
        .getDocuments(testVariables.secondUserId)
        .then(() =>
          done(
            Error(
              "getDocumentsCount rejection didn't work properly in documentService.loginNode"
            )
          )
        )
        .catch(err => {
